...
let self = module.exports = {
  ...
  updateAllElemetsDataBase: function (req, res) {
    ...
    if (res) {
      ...
      console.log("This update running in backgound.");
    }
  ...
  getIsinIdAndHistoryFromPagePart2: function (body, fund, firstReg, res, callback) {
    ...
    if (marketId) {
      ...
      try {
        ...
        urlFundHistCall = urlFundHistCall + marketId.xid;
      } catch (e) {
    ...
    utils.requestExternalWebPage(urlFundHistCall, function (err, resHtml, body) {
      if (err) {
        ...
        try {
          ...
          if (allRecords[0]) {
            let checkDateArr = lastObj.EndDate;
  ...
  getFundCotacaoByDateAndCalcMoney: function (res, isin, nUps, dateCota, dateEnd, callback) {
    Fund.getFundCotacaoByDateAndCalcMoney(res, isin, nUps, dateCota, dateEnd, callback);
  },
  ...
  updateIndividualFund: function (funds, pos, res, callback) {
    if (funds[pos]) {
      self.sendMsgToParentProc("updateFundsAndWallets", true, "", "Step 1 / 3", (pos * 100 / funds.length).toFixed(0) * 1, false);
      ...
      self.sendMsgToParentProc("updateFundsAndWallets", true, "", "Step 1 / 3", 100, false);
