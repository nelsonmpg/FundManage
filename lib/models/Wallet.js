...
Wallet.prototype.updateCotacao = function (res, fund, pos, callback) {
  console.log("Save fund Wallet", pos, fund.owner, fund.Value, fund.isin, fund.name, fund.dateInvest, fund.HistoryDetail.length, fund.HistoryDetail[0], fund.HistoryDetail[fund.HistoryDetail.length - 1]);
  let options = {
    upsert: true
  };
  ...
  this.walletdb.find(query, function (err, wallet) {
    ...
    for (let index = 0; index < wallet[0].listFunds.length; index++) {
      ...
      for (let index2 = 0; index2 < wallet[0].listFunds[index].investList.length; index2++) {
        console.log("num invest", wallet[0].listFunds[index].investList.length);
        ...
        if (wallet[0].listFunds[index].investList[index2]) {
          ...
          console.log("Dentro If", val.moneyFund[val.moneyFund.length - 1], val.moneyFund.length, wallet[0].listFunds[index].isin);
