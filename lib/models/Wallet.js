...
Wallet.prototype.getOwnerWalletFundList = function (req, res) {
  ...
  this.walletdb.find(query, function (err, wallet) {
          ...
          if (portfolio.listFunds[x].investList[y]) {
            ...
            while (lastDate.getTime() < dateEnd) {
              ...
              if (!portfolio.listFunds[x].investList[y].active) {
                let darInative = new Date(portfolio.listFunds[x].investList[y].dateInative)
                if (lastDate.setHours(0, 0, 0, 0) > darInative.setHours(0, 0, 0, 0)) {
                  lastObj.moneyCalc = 0;
                }
              }
    ...
    for (let index = 0; index < arrAuxSorted.length - 1; index++) {
...
Wallet.prototype.updateMoneyFundWallet = function (walletId, dateCheck, fundUpdate, pos) {
  let options = {
    upsert: true
  };
  this.walletdb.updateOne({
    ...
    "listFunds.investList.dateCheck": dateCheck
      ...
      if (err) {
        return console.log("Error to update value cotacao in Wallet." + err);
      }
      console.log("Update Fund Money in Wallet", result /**/);
    })

}

Wallet.prototype.lastUpdateOnlyDateFundInWallet = function (walletId, dateCheck, isin, pos) {
  let options = {
    upsert: true
  };
  this.walletdb.updateOne({
    ...
    "listFunds.investList.dateCheck": dateCheck
      ...
      console.log("Update Date Fund in Wallet", isin /*, result*/);
    });
}
