...
Wallet.prototype.updateCotacao = function (res, fund, callback) {
  ...
  this.walletdb.updateOne({
    ...
    "listFunds.codefund": fund.isin + "-" + new Date(fund.dateInvest).getTime().toString()
      ...
      "$set": {
        "listFunds.0.cotacaoUp": fund.Value,
        "listFunds.0.dateLastUpdate": new Date(),
        "listFunds.0.valInvest": (fund.nUps * fund.Value),
        "listFunds.0.valLastMoney": fund.HistoryDetail[fund.HistoryDetail.length - 1].moneyCalc,
        "listFunds.0.dateLastUpdateInvest": fund.HistoryDetail[fund.HistoryDetail.length - 1].EndDate
      },
      "$push": {
        "listFunds.0.moneyFund": {
