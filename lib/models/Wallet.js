...
Wallet.prototype.updateCotacao = function (res, fund, callback) {
  console.log("Save fund Wallet - code", fund.isin + "-" + new Date(fund.dateInvest).getTime(), fund.Value, fund.isin, fund.dateInvest, fund.HistoryDetail.length, fund.HistoryDetail[0], fund.HistoryDetail[fund.HistoryDetail.length - 1]);
  let options = {
    upsert: true
  };
  this.walletdb.updateOne({
    "owner": fund.owner,
    "listFunds.codefund": fund.isin + "-" + new Date(fund.dateInvest).getTime()
  }, {
      "$set": {
