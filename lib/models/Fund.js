...
Fund.prototype.saveFund = function (req, res, callback) {
  ...
  self.getFundControl(req.body.isin, res, function () {
    isinValidator(req.body.isin, function (err) {
      if (err) {
        if (res) {
          res.status(500).send({
            status: false,
      ...
      // console.log("ISIN is fine!", req.body.isin, req.body.name, (req.body.idFundMorningstar !== "" ? req.body.idFundMorningstar : ""));
      self.funddb.create({
        isin: req.body.isin,
        name: req.body.name,
...
Fund.prototype.updateDateLastCall = function (isin, res, callback) {
  ...
  this.funddb.updateOne(
    {
      ...
      "$set": {
        nameFull: dataFund.fundFullName,
        category: dataFund.morningstarCategory,
        availableSale: dataFund.availableSale,
        rating: dataFund.marketRate,
        currency: dataFund.priceCurrency,
        domicile: dataFund.domicile
      }
    }, options, function (err, result) {
      if (err) {
        if (res) {
