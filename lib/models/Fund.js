...
Fund.prototype.updateHistory = function (isin, idfund, subObj) {
  ...
  this.funddb.updateOne(
    {
      ...
      console.log("Update Fund History.", err, result);
    })
}

Fund.prototype.updateDateLastCall = function (isin) {
  let options = { upsert: true };
  this.funddb.updateOne(
    {
      isin: isin
      ...
      console.log("Date Last Call Update ", err, result);
    })
}
...
// Não utilizada
Fund.prototype.getFundCotacaoByDate = function (owner, isinCode, nUps, dateCota, callback) {
  let self = this;
  let query = [
    {
      $match: { "isin": isinCode }
      ...
      $match: { "HistoryDetail.EndDate": dateCota }
    }, {
      $project: {
        _id: 0,
  ...
  this.funddb.aggregate(query, function (err, fund) {
    if (err) {
      return console.log("There was a problem finding Fund.");
    }
    fund[0].isin = isinCode;
    fund[0].owner = owner;
    fund[0].nUps = nUps;
    callback(fund[0]);
  });
...
Fund.prototype.getFundCotacaoByDateAndCalcMoney = function (res, owner, isinCode, nUps, dateCota, callback) {
  let self = this,
    query = [
      {
        $match: { isin: isinCode }
  ...
  this.funddb.aggregate(query, function (err, fund) {
    ...
    if (fund[0].Value === 0) {
      ...
      return self.getFundCotacaoByDateAndCalcMoney(res, owner, isinCode, nUps, dat, callback);
    }
    fund[0].owner = owner;
    fund[0].nUps = nUps;
    callback(fund[0]);
