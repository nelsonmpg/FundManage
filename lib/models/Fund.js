...
Fund.prototype.updateDateLastCall = function (isin, updateStartDate, res, callback) {
  ...
  this.funddb.updateOne(
    ...
    if (err) {
      ...
      console.log("Date Last Call Update ", isin, result);
    }
    if (callback) {
...
Fund.prototype.updateInfoFund = function (fund, dataFund, res, callback) {
  ...
  this.funddb.updateOne(query, updateVals, options, function (err, result) {
    if (err) {
      ...
      console.log("Update Fund Info.", fund.isin, result);
    }
    if (res) {
      try {
        ...
        console.log("catch - Error res.status"/* , e */);
      }
    }
    if (callback) {
      callback();
...
Fund.prototype.getFundCotacaoByDateAndCalcMoney = function (res, isinCode, nUps, dateCota, dateEnd, callback) {
  // console.log("func receive", isinCode, nUps, dateCota, dateEnd);
  ...
  this.funddb.aggregate(query, function (err, fund) {
    ...
    /* if (!fund[0]) {
      ...
      return self.getFundCotacaoByDateAndCalcMoney(res, isinCode, nUps, dat, dateEnd, callback);
    }  */
    // console.log("Fund return", isinCode, fund[0].HistoryDetail.length);
    // console.log("FF", fund[0]);
    callback(fund[0]);
  });
}
...
Fund.prototype.BackupFunds = function (callback) {
  this.funddb.find({ /* isin: "LU0689624806"  */ }, {
    ...
    let utils = require('./../utils.js'),
      ...
      fileSave = backupFile.funds;
    fileSave = fileSave.replace("##date##", "_" + utils.dateLastUpdateV3(new Date()))
    utils.createFileSaveContent(fileSave, JSON.stringify(funds));
    callback();
  });
}
